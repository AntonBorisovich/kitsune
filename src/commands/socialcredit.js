...
class Socialcredit {
	constructor(kitsune, commands, values){
		...
		this.argsdesc = "<-f> - добавляет свидетеля из фрязина по бокам картинки, что бы обмануть нейронку и заставить её обработать картинку без человеческих лиц\n" +
						"<-h> - как разделять картинку. Если будет `-h`, то картинка разделится по горизонтале, а если без аргумента, то по вертикали (почти всегда не работает)"; // описание аргументов в помоще по конкретной команде
