...
class Socialcredit {
	constructor(kitsune, commands, values){
		...
		this.argsdesc = "<-f> - добавляет свидетеля из фрязина по бокам картинки, что бы обмануть нейронку и заставить её обработать картинку без человеческих лиц\n" +
						"<-h> - Если будет `-h`, то [картинка разделится по горизонтали](https://i.imgur.com/0jCEGA0.jpg), а если без аргумента, [то по вертикали](https://i.imgur.com/elae33g.jpg)"; // описание аргументов в помоще по конкретной команде
